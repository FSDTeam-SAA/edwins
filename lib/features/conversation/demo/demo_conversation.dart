...
import 'package:language_app/features/conversation/demo/demo_test_result.dart';
...
class _DemoConversationState extends State<DemoConversation>
  ...
  int currentExerciseType =
      1; // 1: Translation, 2: Multiple Choice, 3: Fill in blank
  // Viseme data
      ...
      if (nextChar != null) {
        ...
        if (digraph == 'ch') {
          visemes.add(VisemeData(
              'viseme_CH', currentTime, currentTime + phonemeDuration));
          ...
          if (i + 2 < word.length && word[i + 2] == 'h') {
            visemes.add(VisemeData(
                'viseme_SS', currentTime, currentTime + phonemeDuration));
            currentTime += phonemeDuration;
            i += 3;
            continue;
          }
          ...
          visemes.add(VisemeData(
              'viseme_I', currentTime, currentTime + phonemeDuration));
          currentTime += phonemeDuration;
          i += 2;
          continue;
        } else if (digraph == 'ei' || digraph == 'ai') {
          visemes.add(VisemeData(
              'viseme_aa', currentTime, currentTime + phonemeDuration));
          currentTime += phonemeDuration;
          i += 2;
          continue;
        } else if (digraph == 'eu' || digraph == 'äu') {
          visemes.add(VisemeData(
              'viseme_O', currentTime, currentTime + phonemeDuration));
          currentTime += phonemeDuration;
          i += 2;
          continue;
        }
...
// German words for "Die Katze frisst Hühnchen" - Adjusted for speech rate 0.4
  ...
  void _startLipSync(String text) {
    ...
    for (var word in words) {
      word =
          word.replaceAll(RegExp(r'[^\wäöüß]'), ''); // Keep German characters
    ...
    Timer(const Duration(seconds: 3), () {
      if (mounted) {
        Navigator.pushReplacement(
          context,
          _createFadeRoute(const DemoTestResult()),
        );
      }
    });
  }
              ...
              children: [
                Expanded(
                  child: _AnimatedOptionButton(
                    ...
                    onPressed: () => _checkAnswer(
                        options[0]['text'], options[0]['isCorrect']),
                ...
                Expanded(
                  child: _AnimatedOptionButton(
                    text: options[1]['text'],
                    onPressed: () => _checkAnswer(
                        options[1]['text'], options[1]['isCorrect']),
              ...
              children: [
                Expanded(
                  child: _AnimatedOptionButton(
                    text: options[2]['text'],
                    onPressed: () => _checkAnswer(
                        options[2]['text'], options[2]['isCorrect']),
                ...
                Expanded(
                  child: _AnimatedOptionButton(
                    text: options[3]['text'],
                    onPressed: () => _checkAnswer(
                        options[3]['text'], options[3]['isCorrect']),
    ...
    return AnimatedContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          if (showRecordingControls) _buildRecordingControls(),
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color:
                    _isPressed ? const Color(0xFFFF609D) : Colors.grey.shade300,
                    ...
                    gradient: LinearGradient(
                      colors: [
                        ...
                        const Color(0xFFFF8E8E)
                            .withOpacity(0.9), // Even lighter red
                      ],
                    ),
                    borderRadius: BorderRadius.circular(16),
                  ),
